...
class KnightMoves(unittest.TestCase):
    ...
    def testKnightCenterJumps(self):
        board = ChessBoard()
        board.addPieceToBoard("Knight", 4, 4, 0)
        board.addPieceToBoard("Pawn", 3, 4, 0)
        board.addPieceToBoard("Pawn", 3, 5, 0)
        board.addPieceToBoard("Pawn", 4, 5, 0)
        board.addPieceToBoard("Pawn", 5, 5, 0)
        board.addPieceToBoard("Pawn", 5, 4, 0)
        board.addPieceToBoard("Pawn", 5, 3, 0)
        board.addPieceToBoard("Pawn", 4, 3, 0)
        board.addPieceToBoard("Pawn", 3, 3, 0)
        moves = board.getRank(4, 4).getMoveset(board.getRanks())
        self.assertEqual(8, len(moves))
        self.assertIn((2, 3), moves)
        self.assertIn((2, 5), moves)
